...
If the API returns **200** but **`git push` still says “Repository not found”**, common causes were **`https://x-access-token:…`** (only for Actions `GITHUB_TOKEN`), **embedding `user:token` in the URL**, or **credential-helper** not picking up the PAT. The workflow pushes with **`Authorization: Basic` (base64 of `login:token`)** via **`git -c http.extraHeader=…`**, and uses **`${CLIENT}` / `${SERVER}`** from secrets (never hardcoded repo names).
